---
description: How validators and node operators apply the v2.0.0 hard fork, in two phases.
...
This upgrade is in preparation. The documentation on this page is published as a preview so you can review the changes and prepare ahead of activation. This notice is updated as the status changes.
::

The v2.0.0 hard fork contains security-relevant improvements to the protocol. The rollout happens in two phases with different audiences, actions, and timing. Read the section for your role.
...
Validators will receive the security patch through the CERT channel. The patch contains the hard fork code.
...
Validators are finished after this phase. The Phase 2 public release is not needed for validators.
::

## Phase 2: Node operators

Phase 2 starts only after the chain has been upgraded, immediately following the fork. A public release is published at that point and announced in the Coders Dojo Telegram channel. Node operators who want to follow the upgraded chain must apply it. Validators do not need to, since they already applied the patch.
