...
> @nimiq/core / Transaction

[@nimiq/core](../globals) / Transaction
...
delegation from one validator to another. To create such a transaction, set `flags` to ` 0b10` and populate the `data` bytes accordingly.
...
The transaction's [TransactionFormat](../enumerations/TransactionFormat).
...
The transaction's recipient [AccountType](../enumerations/AccountType).
...
The transaction's sender [AccountType](../enumerations/AccountType).
...
`string` | [`PlainTransaction`](../interfaces/PlainTransaction) | `Uint8Array`<`ArrayBufferLike`> | `Transaction`
