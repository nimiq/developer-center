...
Every other node type does not use State Live Sync. Instead, they transition to **Block Live Sync** to synchronize both blocks and complete account state simultaneously.
...
At the end of the process, the node compares its computed **state root** against the expected root in the latest block to verify correctness. This approach eliminates the need for transaction history while maintaining complete account state.

## Architecture Overview

<collapsible title="State Live Sync Struct (click to expand)">
...
<tbody>
  <tr>
    ...
    <td>
      Main sync controller implementing <code>
        Stream
      </code>
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      Manages state chunk requests and buffering for trie reconstruction
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      Handles tree diff processing and application for incremental updates
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      Manages block buffering, ordering, and gap detection
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      Handles network requests for state chunks
    </td>
  </tr>
</tbody>
</table>
