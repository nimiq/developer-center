...
<tbody>
  <tr>
    ...
    <td>
      Minimum 16GB RAM (higher recommended)
    </td>
    ...
    <td>
      Starts from a few gigabytes, grows linearly with blockchain size
    </td>
    
    <td>
      High-speed, reliable internet connection; Good I/O performance (SSDs required)
    </td>
  ...
  <tr>
    ...
    <td>
      Minimum 80GB of storage, 160GB recommended
    </td>
    
    <td>
      High-speed, reliable internet connection; Good I/O performance (SSDs recommended)
    </td>
    
    <td>
      Sync time grows linearly but slowly
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      Works with minimal storage
    </td>
    
    <td>
      Moderate-speed internet connection (e.g., 1 Mbps or higher)
    </td>
...
1. [`getTransactionByHash`](/rpc/methods/get-transaction-by-hash): Retrieve transaction details using a transaction hash.
2. [`getTransactionHashesByAddress`](/rpc/methods/get-transaction-hashes-by-address): Fetch transaction hashes associated with a given address.
3. [`getBlockNumber`](/rpc/methods/get-block-number): Returns the current head block of the node.
4. [`getBlockByNumber`](/rpc/methods/get-block-by-number): Returns the block at the specified height
5. [`getRawTransactionInfo`](/rpc/methods/get-raw-transaction-info): Get raw transaction details by decoding a serialized transaction.
6. [`isConsensusEstablished`](/rpc/methods/is-consensus-established): Returns a boolean specifying if the node has established consensus with the network.
7. [`createBasicTransaction`](/rpc/methods/create-basic-transaction): Create a signed transaction and returns a hex-encoded representation without broadcasting it. All `create*Transaction` RPC methods also have an equivalent `send*Transaction` version in order to create and broadcast the transaction one-go.
8. [`sendRawTransaction`](/rpc/methods/send-raw-transaction): Sends the given serialized and signed transaction to the network.
...
4. Run the client via Docker.
  ...
  - For history node on **mainnet**:
    ...
    - Run the client with the `NIMIQ_OVERRIDE_MAINNET_CONFIG` environment variable:```text
  ...
  ```
  - For other configurations (**non-history** nodes or **non-mainnet**):```text
...
<tbody>
  <tr>
    ...
    <td>
      Incoming network connections port
    </td>
  </tr>
...
Besides the `nimiq-client` we provide a set of other binaries to facilitate interactions with the network and simplify key tasks. To run these binaries, you need to [compile them from source](#running-the-nimiq-client-%E2%80%94-from-source-code-ubuntu).

- `nimiq-address`: Generates a Nimiq address and corresponding key pair. Outputs the user-friendly address, raw address, public key and private key.```bash
cargo run --release --bin nimiq-address -- --help
cargo run --release --bin nimiq-address
```
- `nimiq-signtx`: Signs a transaction using key parameters such as sender address, recipient address, value and fee. Outputs the signed transaction in hexadecimal format.```bash
...
```
- `nimiq-rpc`: A CLI RPC client that can query all the supported JSON-RPC methods.```bash
...
<tbody>
  ...
  <tr>
    ...
    <td>
      After the next macro block (end of every batch)
    </td>
  </tr>
</tbody>
</table>
...
- Albatross White Paper: [https://arxiv.org/pdf/1903.01589](https://arxiv.org/pdf/1903.01589)
- Protocol: [https://www.nimiq.com/developers/protocol/](https://www.nimiq.com/developers/protocol/)
