...
Our approach consists of targeting a specific block height within the PoW chain designated as the *transition block*. At this block height, the blockchain state will be captured and used to generate the "genesis block" for the PoS chain. Instead of starting from block height 0, both chains will share the same block number. The PoS chain starts once the conditions covered in this document are met.
...
3. Once 10 confirmation blocks are mined, registered validators capture the state of block 3 456 000 and start generating the "genesis" block.
4. Validators generate the "genesis" block deterministically. The block hash should be the same across all validators.
...
6. The network monitors readiness transactions within a 24 hour window.
7. If 80% of the total stake (represented by registered validators) sends readiness transactions within 24 hours, the PoS chain starts at block height 3 456 000. If the threshold is not reached, the process automatically moves to the next activation window.
...
9. In subsequent activation windows, validators regenerate the "genesis" block (3 457 440), send readiness transactions again, and the process repeats until 80% readiness is achieved.
...
The `GenesisConfig` is generated after the candidate block is mined. If the pre-registered validators run the tool before the activation window starts, the tool will have already migrated most of the history by the time the candidate block is mined. After the candidate block is mined and the network reaches 80% readiness, the tool migrates the final part of the history and generates the "genesis" block, which becomes the transition block for the PoS chain.

The activation tool activates at the candidate block - the block height aligned with an election block corresponding to the PoS chain’s structure. At this point, the tool scans and analyzes transactions sent to the network. The transition starts if at least 80% of the total stake is ready.
...
Once 80% readiness is achieved and the final portion of the history is migrated, the transition block is finalized, marking the official beginning of the PoS chain. All PoW blocks mined after the transition block are disregarded as if they never occurred. From this point forward, the network operates entirely under the PoS consensus mechanism.
