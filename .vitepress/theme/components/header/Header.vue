...
<template>
  <div border-bottom lg="fixed top-0" w-full bg-neutral-0 z-100>
    <header mx-auto flex="~ items-center" class="nq-raw">
      <a :href="withBase('/')" focusable flex="~ items-center gap-8" p-6 ml--6 un-text="19 md:20 neutral" relative>
        <ContextMenu.Root>
          <ContextMenu.Trigger as-child aria-label="Nimiq Developer Center">
            <div class="dark:i-nimiq:logos-nimiq-white-horizontal i-nimiq:logos-nimiq-horizontal" text-18 />
            <h2 whitespace-nowrap hidden xs:block w-max font-100 text-16 top--1 relative>
          ...
          <ContextMenu.Portal>
            ...
            >
              ...
              <ContextMenu.Item
                ...
                hocus:bg-neutral-100 transition-colors px-8 py-4 rounded-2 nq-arrow after:opacity-70
              >
        ...
        <div v-if="showEnvironment" absolute top--12 right--16 @click.prevent>
          <Environment />
        </div>
      </a>
      <ClientOnly>
        ...
        <template v-else>
          <SearchBox v-if="showSearch" @close="() => showSearch = false" />
          <button
            ml-32 text-14 py-6 px-12 nq-input-box max-w-320 min-w-192 group flex="~ gap-8 items-center" rounded-full
          ...
          >
            ...
            <div
              ...
              ring="1 neutral/10 group-hocus:blue-600" ml-auto font-medium
