    ...
    <header mx-auto flex="~ items-center" class="raw">
      <a :href="withBase('/')" focusable flex="~ items-center gap-10" p-6 ml--6 un-text="19 md:20 neutral" relative>
        <ContextMenu.Root>
          ...
          <ContextMenu.Portal>
            <ContextMenu.Content :side-offset="8" bg-neutral-0 ring="1 neutral/20" z-100 rounded-6 p-8 shadow
              flex="~ col gap-8">
              <ContextMenu.Item v-if="isSupported" as="button" flex="~ items-center gap-8" hocus:bg-neutral-100
                transition-colors px-8 py-4 rounded-2 @click="copy(nimiqLogo)">
                <div i-nimiq:copy op-80 text-14 />Copy Logo
              </ContextMenu.Item>
              <ContextMenu.Item as="a" :href="withBase('/build/ui/design/logo')" flex="~ items-center gap-8"
      ...
      <ClientOnly>
        ...
        <template v-else>
          ...
          <button ml-32 text-14 py-6 px-12 input-box max-w-320 min-w-192 group flex="~ gap-8 items-center" rounded-full
            ...
            <div flex="~ items-baseline gap-4" pointer-events-none mr--3 mb--1 py-2 lh="11" select-none rounded-full
