...
<template>
  <nav
    v-if="frontmatter.footer !== false" grid="~ cols-[auto_auto] justify-between gap-40" pt-96
    :class="!hasPrev && !hasNext ? 'grid-rows-1' : 'grid-rows-2'"
  >
    ...
    <a v-if="hasPrev" :href="prev.link" blue-pill group>
      <div i-nimiq:arrow-left w-10 group-hover="translate-x--2" duration-100ms />
      {{ prev.text }}
    </a>

    <a v-if="hasNext" :href="next.link" justify-self-end blue-pill group>
      {{ next.text }}
