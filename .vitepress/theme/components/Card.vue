...
const hasColor = computed(() => props.item.bgColor)
</script>

<template>
  <a :href="item.href" :data-inverted="item.bgColor" flex flex-col relative h-full cursor-pointer rounded-6 overflow-hidden :class="{
    ...
    'p-24': isNormal || !hasBgIcon, 
    'p-40 pb-80': hasBgIcon ,
    'p-40': isIcon,
    'items-center': centered,
  }" transition hover="-translate-y-6 shadow" class="group">
    <div :class="{
      'absolute inset-y-50 sm:-right-16 lg:-right-32 -right-64 w-[15vw] children:w-full children:h-full': !centered,
      'fill-[#0E65C9]': item.bgColor === 'blue',
    ...
    <p v-if="item.description" text="darkblue-50 inverted:white/60" mt-20 z-10 inverted:max-w-256 :class="{'text-center': centered}">
      {{ item.description }}
    </p>
